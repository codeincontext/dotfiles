...
set -g fish_prompt_git_status_order added modified renamed copied deleted
untracked unmerged
...
function git_prompt --description 'Write out the git prompt'
  ...
  set -l index (git status --porcelain ^/dev/null|cut -c 1-2|sort
  -u)
...
untracked
    ...
    unmerged
            ...
            staged[1]
                set_color
                $fish_color_git_staged
                  else
                      set_color
                      $fish_color_git_dirty
                        end
